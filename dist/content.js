...
function extractReadableText(contentHtml, fallbackText) {
...
function extractBlockText(contentHtml) {
    if (!contentHtml.trim()) {
        return "";
    }
    const parser = new DOMParser();
    const doc = parser.parseFromString(contentHtml, "text/html");
    const blocks = Array.from(doc.body.querySelectorAll("h1, h2, h3, h4, p, li, blockquote, pre, figcaption"));
    const lines = blocks
...
class Overlay {
    ...
    bindInteractiveElements() {
        ...
        const followUpButtons = Array.from(this.panel.querySelectorAll(".ic-followup"));
        ...
        ${data.questionResult.followUps.length > 0
            ? `
              <div class="ic-followups">
                ...
                .map((followUp) => `<button class="ic-followup" type="button" data-followup="${this.escapeHtml(followUp)}">${this.escapeHtml(followUp)}</button>`)
                .join("")}
              </div>
            `
            : ""}
